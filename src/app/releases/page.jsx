...
import { ArrowRight, CalendarDays, Clock3, PlayCircle, Sparkles, Ticket } from 'lucide-react';
...
const upcomingReleases = [
  {
    ...
    blurb: 'A lyrical thriller about memory, inheritance, and the secrets hidden beneath a weathered estate.',
  ...
  },
  {
    ...
    blurb: 'A sun-kissed story of second chances, told across one unforgettable coastal weekend.',
  ...
  },
  {
    ...
    status: 'Early Access',
    genres: ['Action', 'Crime'],
    blurb: 'When the city slips into blackout, one courier becomes the last line of defense.',
  ...
  },
  {
    ...
    blurb: 'An enchanted heist unfolds as a young thief uncovers a map to a hidden kingdom.',
    accent: 'from-slate-500/20 to-slate-400/5',
  },
];

const releaseTimeline = [
  { phase: 'VIP Preview', date: 'Aug 24', detail: 'First look and red carpet access' },
  { phase: 'Official Trailer', date: 'Sep 01', detail: 'New clips and exclusive behind-the-scenes' },
  { phase: 'Opening Weekend', date: 'Sep 19', detail: 'Priority booking unlocks for members' },
  { phase: 'Extended Run', date: 'Oct 03', detail: 'Late-night screenings and special events' },
];
        ...
        <div className="relative z-10 grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <Reveal as="div" className="max-w-2xl">
            ...
            <p className="mt-4 max-w-xl text-base text-on-surface-variant sm:text-lg">
              Explore the next wave of premieres, watch the momentum build, and reserve your spot before the crowd arrives.
            </p>

            <div className="mt-8 flex flex-wrap gap-3">
              <Button className="rounded-full px-5 py-3">Reserve premiere access</Button>
              <Link
                href="/movies"
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              {[
                ...
                <div key={label} className="glass flex items-center gap-2 rounded-full px-3 py-2 text-sm text-on-surface-variant">
            ...
            <div className="release-card glass relative overflow-hidden rounded-[1.75rem] border border-white/10 p-4 shadow-2xl shadow-crimson/10">
              ...
              <div className="relative overflow-hidden rounded-[1.3rem]">
                ...
                <div className="absolute inset-x-0 bottom-0 p-5">
                  ...
                  <h2 className="font-display text-2xl font-bold text-white">{spotlightRelease.title}</h2>
                  <p className="mt-2 text-sm text-on-surface-variant">{spotlightRelease.tagline}</p>
                  <div className="mt-4 flex flex-wrap gap-2 text-xs text-on-surface-variant">
                    {spotlightRelease.genres.map((genre) => (
                      <span key={genre} className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1">
                        {genre}
                      </span>
                    ))}
                  </div>
            ...
            {upcomingReleases.map((release, index) => (
              <Reveal key={release.title} delay={index * 70}>
                <article className="release-card group rounded-[1.3rem] border border-white/[0.08] bg-white/[0.03] p-5 transition-all duration-500 hover:bg-white/[0.05]">
                  <div className={`rounded-[1rem] border border-white/10 bg-gradient-to-br ${release.accent} p-4`}>
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-primary">
                          {release.status}
                        </p>
                        <h3 className="mt-2 font-display text-xl font-semibold text-white">{release.title}</h3>
                      </div>
                      <div className="rounded-full border border-white/10 bg-obsidian/70 px-3 py-1 text-sm font-medium text-on-surface-variant">
                        {release.date}
                      </div>
                    </div>
                    <p className="mt-4 text-sm text-on-surface-variant">{release.blurb}</p>
                    <div className="mt-4 flex flex-wrap gap-2">
                      {release.genres.map((genre) => (
                        <span key={genre} className="rounded-full border border-white/10 bg-white/[0.06] px-2.5 py-1 text-[11px] uppercase tracking-[0.2em] text-on-surface-variant">
                          {genre}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex gap-3 rounded-[1rem] border border-white/[0.06] bg-white/[0.03] p-4">
                    ...
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="text-xs uppercase tracking-[0.2em] text-gold">{item.date}</span>
                      </div>
          ...
          <Reveal>
            <div className="relative overflow-hidden rounded-[1.75rem] border border-white/[0.08] bg-gradient-to-br from-teal/15 via-obsidian to-crimson/10 p-6 md:p-8">
              ...
              <div className="relative z-10">
                <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-teal">Members-only perk</p>
                <h3 className="mt-2 font-display text-2xl font-semibold text-white">Be first in line for immersive premieres.</h3>
                <p className="mt-3 text-sm text-on-surface-variant">
                  Unlock early access, curated reminders, and a smoother checkout experience for every launch week.
                </p>
                <div className="mt-5 flex flex-wrap gap-3">
                  <Button className="rounded-full px-4 py-2.5">Join the waitlist</Button>
                  <Link href="/login" className="inline-flex items-center rounded-full border border-white/10 bg-white/[0.04] px-4 py-2.5 text-sm font-semibold text-on-surface transition hover:bg-white/[0.06]">
                    Sign in
                  </Link>
                </div>
              </div>
